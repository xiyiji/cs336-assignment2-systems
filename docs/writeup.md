...
| model | ctx | forward | forward + backward | full step (fwd + bwd + AdamW) |
|---|---|---|---|---|
| small (128.6 M) | 128 | 361 ± 195 ms | 1030 ± 116 ms (fwd 270 / bwd 748) | 1762 ± 322 ms (opt 680) |
| small | 256 | 501 ± 27 ms | 1886 ± 254 ms (fwd 556 / bwd 1318) | 2654 ± 383 ms (opt 458) |
| small | 512 | 2954 ± 1481 ms | 6600 ± 1384 ms (fwd 2316 / bwd 4261) | 8921 ± 2542 ms (opt 1096) |
| medium (352 M), 2 warm-up / 5 steps | 128 | – | 5605 ± 919 ms (fwd 1278 / bwd 4239) | – |

A forward pass of `small` at ctx 256 takes ~0.5 s on this CPU and the
backward ~2.4× that (it has 2× the matmul FLOPs plus the activation-gradient
elementwise work); the AdamW step of the assignment-1 (unfused, per-tensor)
optimizer adds another 0.5–1 s for 129 M parameters. Variability is high —
±10–25 % at ctx 256, up to ±50 % at ctx 512 where the 8-core machine starts
to thermally throttle and the working set exceeds the caches — much more than
one would see on a GPU, where step-to-step jitter is typically < 2 %.
(`medium` and the larger sizes were not swept: `xl` alone is 3.4 B parameters,
13.6 GB of fp32 weights, and a single CPU step would take minutes.)

**(c)** Warm-up ablation (`small`, ctx 256, forward+backward, 10 timed steps):

| warm-up steps | mean ± std over 10 timed steps | first timed step |
|---|---|---|
| 0 | 1943 ± 255 ms | 1746 ms |
| 1 | 1834 ± 216 ms | 2373 ms |
| 2 | 2345 ± 877 ms | 1977 ms |
| 5 | 1886 ± 254 ms | 1905 ms |

On this CPU the warm-up count makes no difference beyond run-to-run noise:
the four means agree within their standard deviations and the first timed step
is not systematically slower. That is expected on CPU — eager PyTorch has no
lazy device initialisation, kernel loading or allocator warm-up to amortise
(only the OpenMP thread pool, a few ms). On a GPU the same experiment looks
very different: the first iteration pays for CUDA context creation,
cuBLAS/cuDNN handle creation and heuristic selection, loading the kernel
modules, and the caching allocator growing its pool, so with 0 warm-up steps
the mean is inflated and the standard deviation explodes. With only 1–2
warm-up steps GPU numbers can *still* differ from the steady state because
some one-time costs happen on later steps — the optimizer's first `step()`
allocates its state, the allocator keeps growing until the steady-state
working set has been touched once, `torch.compile`/cuDNN autotuning retune
on the first few shapes, and GPU clocks take several hundred ms to boost —
which is why the handout's 5 warm-up steps is a safer default.
...
| overlap + sharded optimizer (§6) | 3259 ± 304 | 44 | 1.4 % | 491 MiB | **491 MiB** |
| FSDP (§7; collectives serialised on macOS/gloo, see below) | 3268 ± 541 | 0 (all waited inline) | – | **245 MiB** | **491 MiB** |
...
| setting (2 ranks, `small`, ctx 128) | step | local params | local AdamW state |
|---|---|---|---|
| DDP overlap, full AdamW on every rank | 3999 ± 815 ms | 491 MiB | 981 MiB |
| DDP overlap + `ShardedOptimizer(AdamW)` | 3259 ± 304 ms | 491 MiB | 491 MiB |
| FSDP | 3268 ± 541 ms | 245 MiB | 491 MiB |

(`small` has 128.6 M parameters: 491 MiB fp32 weights, the same again for
gradients, 2× for AdamW's `m`/`v`. On a GPU the same three numbers are read
from `torch.cuda.max_memory_allocated()` at the three points; the CPU run
reports exact byte counts instead.)

**(b)** Speed: on this CPU the sharded step was *faster* (3259 vs 3999 ms,
within the ±20 % noise): each rank runs the assignment-1 AdamW (a chain of
unfused elementwise ops, 0.5–2 s per step for 128 M parameters on CPU) on
only half of the parameters, which outweighs the 491 MiB parameter broadcast
that the step now ends with (~0.2 s at gloo's ~2.5 GB/s). On NCCL the
optimizer step is tiny and the extra `Ψ·4 B` broadcast (13.6 GB for `xl`) is
pure overhead unless it is overlapped with the next forward, so there the
handout's expectation — slightly slower per step, much less memory — holds.
...
*and* optimizer-state bytes halving (245 MiB + 491 MiB vs 491 MiB + 981 MiB,
table above) at a step time equal to the sharded-optimizer DDP run
(3268 vs 3259 ms).

*Implementation note (macOS only).* gloo's libuv transport corrupts its
stream ("Unexpected opcode … pair.cc") when many asynchronous
`all_gather_into_tensor`/`reduce_scatter_tensor` calls are in flight, which
killed the first FSDP run on the `small` model. `FSDP(sync_comm=True)` — the
default on macOS+gloo, `CS336_FSDP_SYNC_COMM=0` to override — waits on each
collective right after issuing it. Linux gloo (CI) and NCCL use the fully
asynchronous, prefetching path, which is the one the tests in CI exercise.
